...
def render_tab_similarity(df, intents, sidebar_cfg):
    ...
    if phrase_confusion:
        # Sort by similarity descending so the most critical conflicts appear first.
        phrase_confusion_sorted = sorted(phrase_confusion, key=lambda c: float(c['Similarity']), reverse=True)
        with st.spinner(f"Building recommendations for {len(phrase_confusion_sorted):,} phrase conflicts..."):
